...
def register(data: RegisterSchema, db: Session = Depends(get_db)):
    ...
    u = User(
        ...
        phone=data.phone,
        login=data.login,
        password=data.password,  # без хеша (как у тебя)
        role_id=role.id
    )
    db.add(u)
    db.commit()
...
def create_request(data: RequestCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    r = Request(
        ...
        status_id=st_id,
        client_id=user.id if rn == "client" else user.id,  # если надо — можно расширить
        master_id=None,
...
def update_request(request_id: int, data: RequestUpdate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    # 2.2 редактирование: статус / описание / ответственный
    rn = role_name_by_id(db, user.role_id)
    ...
    # клиент может менять только описание своей заявки (если очень надо); чаще запрещают — оставим минимум:
    if rn == "client":
    ...
    if data.master_id is not None:
        # назначение специалиста (2.4)
        r.master_id = data.master_id

    if data.status is not None:
        r.status_id = status_id_by_name(db, data.status)
        # если завершили — ставим completion_date если не задана
...
def add_comment(data: CommentCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    # специалист — только по своим заявкам
...
def stats(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    # среднее время: completion_date - start_date (только где completion_date есть)
    # берем в днях
    ...
    # “типы неисправностей” у нас в problem_description — сделаем топ по ключевым словам (очень простая, но рабочая статистика)
    # считаем по первому слову/фразе до пробела — чтобы был топ
